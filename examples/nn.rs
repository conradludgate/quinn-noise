...
use noise_protocol_quinn::{
    noise_protocol::{patterns::noise_nn, HandshakeStateBuilder},
...
async fn server(endpoint: quinn::Endpoint) -> Result<()> {
    loop {
        ...
        // during NN handshake, the server does not know the client's static key
...
pub async fn connect_client(
    ...
    // during NN handshake, the client does not know the server's static key
    assert!(connection.peer_identity().is_none());
